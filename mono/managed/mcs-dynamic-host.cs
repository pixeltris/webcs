// Using mcs.exe directly has some issues... e.g. StaticLoader ctor fails on empty path (mono paths are located at '/')
// https://github.com/aeroson/mcs-ICodeCompiler
// - mcs.dll (renamed to mcs-dynamic.dll) taken  from tree (Nov 29, 2018) https://github.com/aeroson/mcs-ICodeCompiler/tree/4dde7e946ad1c69c7f9e5b87d52c4d0cfdacb27a
// csc /r:mcs-dynamic.dll mcs-dynamic-host.cs
using System;
...
class Program
{
    static void Main()
    {
        ...
        // NOTE
        ///////////////////////////////////////////////
        // - We can't save assemblies using this as the mono wasm builds set DISABLE_REFLECTION_EMIT_SAVE
        // - If we want to save we will need to use IKVM.Reflection - see mono/mcs/docs/compiler.txt
        // - Using mcs.exe directly (IKVM.Reflection) has it's own issues in StaticLoader ctor (see above)
        ///////////////////////////////////////////////
        
        string[] files = {"hello2.cs"};
        string assemblyName = "test";
        bool generateInMemory = true;
        File.WriteAllText(files[0], @"public class Program
{
    public static void Main()
    {
        ...
        settings.OutputFile = assemblyName + settings.TargetExt;
        ...
        foreach (string file in files)
        {
            ...
            settings.SourceFiles.Add(new SourceFile(fileInfo.Name, fileInfo.FullName, settings.SourceFiles.Count + 1));
        }
        
        CustomDynamicDriver driver = new CustomDynamicDriver(new CompilerContext(settings, new ConsoleReportPrinter()));
        
        System.Reflection.Emit.AssemblyBuilder outAssembly = null;
        driver.Compile(out outAssembly, AppDomain.CurrentDomain, generateInMemory);
        
        Console.WriteLine("assembly:" + outAssembly);
    ...
    public class CustomDynamicDriver
    {
        ...
        public bool Compile(out AssemblyBuilder outAssembly, AppDomain domain, bool generateInMemory)
        {
           ...
           /* if (output_file == null)
            ...
            /*foreach (var v1 in outAssembly.GetTypes())
            {
                Console.WriteLine(v1.Name);
                if (v1.Name == "Program")
                {
                    v1.GetMethod("Main").Invoke(null, null);
                }
            }*/
